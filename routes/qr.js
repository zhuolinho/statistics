...
router.get('/', function (req, res, next) {
    if (querying || req.connection.remoteAddress.split(":")[3] != "211.161.198.70") res.send("querying...");
        ...
        connection.connect();
        connection.query("SELECT client_id, COUNT(*) AS count, SUM(actual_fee) AS sum FROM tb_park_charge_order WHERE crt_time >= '" + req.query.startDate + "' AND crt_time < '" + req.query.endDate + "' AND order_category = 'QR' AND ispay = 'Y' AND STATUS = 'R' GROUP BY client_id", function (error, results, fields) {
            if (error) throw error;
            var orders = {};
            results.forEach(function (order) {
                orders[order.client_id] = order;
            });
            connection.query("SELECT a.client_id, b.name, b.phone, a.c, a.s, c.name AS park_name FROM (SELECT client_id, COUNT(*) AS c, SUM(price) AS s, park_id FROM tb_park_cost_trade WHERE create_time >= '" + req.query.startDate + "' AND create_time < '" + req.query.endDate + "' AND TYPE = 'CONSUME' GROUP BY client_id) AS a, tb_park_user AS b, tb_park_park AS c WHERE a.client_id = b.id AND a.park_id = c.id", function (error, results, fields) {
                if (error) throw error;
