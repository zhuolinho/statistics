...
router.get('/', function (req, res, next) {
    ...
    connection.query("SELECT client_id, COUNT(*) AS count, SUM(actual_fee) AS sum FROM tb_park_charge_order WHERE crt_time BETWEEN '2017-05-29' AND '2017-06-05' AND order_category = 'QR' AND ispay = 'Y' AND STATUS = 'R' GROUP BY client_id", function (error, results, fields) {
        ...
        connection.query("SELECT a.client_id, b.name, b.phone, a.c, a.s FROM (SELECT client_id, COUNT(*) AS c, SUM(price) AS s FROM tb_park_cost_trade WHERE create_time BETWEEN '2017-05-29' AND '2017-06-05' AND TYPE = 'CONSUME' GROUP BY client_id) AS a, tb_park_user AS b WHERE a.client_id = b.id", function (error, results, fields) {
            if (error) throw error;
            var str = "client_id,保安,联系方式,当面付支付笔数,当面付支付金额,该保安总临停支付笔数,该保安总临停金额";
            results.forEach(function (trade) {
                ...
                str = str + "\n" + trade.client_id + "," + trade.name + "," + trade.phone + "," + count + "," + sum + "," + (trade.c + count) + "," + (sum - trade.s);
            });
            res.send(str);
            connection.end();
        });
